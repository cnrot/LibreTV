...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新页面标题
        document.title = `DBGTV - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'DBGTV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'DBGTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'DBGTV-Settings_' + times + '.json');
}
